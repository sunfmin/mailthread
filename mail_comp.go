...
func initMailComp() {
	...
	re8 := `On .* wrote:`
	// 在 2013年9月27日星期五，孙 凤民 via Qortex 写道：
	re9 := `在.*写道：`
	// ------------------ 原始邮件 ------------------
	// **发件人:** "孙凤民";
	...
	headComp.re = fmt.Sprintf(`(^(%s|%s|%s|%s|%s|%s|%s|%s|%s|%s) *?\n)`, re1, re2, re3, re4, re5, re6, re7, re8, re9, qqReCn)
	headComp.bareRe = fmt.Sprintf(`((%s|%s|%s|%s|%s|%s|%s|%s|%s|%s) *?\n)`, re1, re2, re3, re4, re5, re6, re7, re8, re9, qqReCn)
